...
int main (void)
{
    ...
    //printf(" -05d: %-05d\n", num);      //флаг '0' игнорируется, когда он вместе с фо=лагом '-'
    printf(" 05d: %05d\n", num);
    printf(" 05d: %05d\n", num1);
    ...
    printf(" .8f: %.8f\n", num3);       
    printf(" .8d: %.8d\n", num);        //для целых чисел заполняет нулями до ширины
    printf(" .8d: %.8d\n", num2);       //число не обрезает
    printf(" .8d: %.8d\n", num2); 
    printf(" .8s: %.8s\n", str);        //строку обрезает
    ...
    printf("10.4d: %10.4d\n", num);     //точность вместе с шириной может быть только у чисел
